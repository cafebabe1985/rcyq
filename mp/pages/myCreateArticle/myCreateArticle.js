...
Page({
  ...
  goNewsDetail(e) {
    ...
    if (displayType == '3') {
      wx.navigateTo({
        url: `/pages/createArticle/createArticle?id=${this.data.openId}&scenicId=${id}`,

      })
    } else {
      wx.navigateTo({
        url: `/pages/articleDetail/articleDetail?id=${id}&type=${3}&path=`,
  ...
  async listNews(query) {
    ...
    const resScenicData = await Article.listAllArticle(1, 1000, 'createTime', 'desc', query)
  ...
  createNews() {
    ...
    wx.navigateTo({
      url: `/pages/createArticle/createArticle?id=${this.data.openId}`,
    })
  },
  /**
   * 生命周期函数--监听页面加载
