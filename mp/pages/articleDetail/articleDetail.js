...
Page({
  ...
  data: {
    ...
    detailId: null,
    modelType: null,
    type: ['scenic', 'active', 'news','article'],
    detail: null,
    content: null,
    wxUser: null,
    comments: null,
  ...
  async onLoad(opt) {
    ...
    this.data.queryPath = `id=${opt.id}&type=3`
    if (opt.path) {
      this.setData({
        pagePath: opt.path
      })
  ...
  async getDetail(id, type) {
    ...
    let detail = null
      ...
      detail = await Article.getArticleById(id)
  ...
  onShareAppMessage(e) {
    ...
    return {
      ...
      path: `/pages/articleDetail/articleDetail?${this.data.queryPath}`,
